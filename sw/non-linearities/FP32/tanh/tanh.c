...
#  elif (TANH_DEG == 3)
        ...
        asm volatile("vfadd.vf        v24, v24, %[kA1]" :: /*
 * Copyright (C) 2021 ETH Zurich
 * and University of Bologna
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
static inline void vtanh_m8_strip(const float* inp, float* out, int N) {
    ...
    while (remaining > 0) {
        ...
        /* Load and get sign/abs */
        asm volatile("vle32.v   v0, (%0)" :: "r"(pin) : "memory");
        asm volatile("vfmv.v.f  v8,  %0" :: "f"(1.0f));      /* keep: needed for vfsgnj */
        asm volatile("vfsgnj.vv v8,  v8,  v0");              /* sign(x) -> {+1,-1} */
        asm volatile("vfsgnjx.vv v0,  v0,  v0");             /* |x| */
        ...
        /* P(r) via Horner (vector–scalar FMAs where applicable) */
#  if   (TANH_DEG == 2)
        asm volatile("vfmv.v.f        v24, %[kA1]" :: [kA1]"f"(A1));
        asm volatile("vfmacc.vf       v24, %[kA2], v0" :: [kA2]"f"(A2));
        asm volatile("vfmul.vv        v0,  v0,  v24");
...
#  endif
        ...
        /* tanh core: v24 = 1 - 2/(E+1) using vector–scalar ops */
        asm volatile("vfadd.vf        v24, v0,  %[one]" :: [one]"f"(1.0f));      /* E+1 */
        asm volatile("vfrdiv.vf       v24, v24, %[two]" :: [two]"f"(2.0f));      /* 2/(E+1) */
        asm volatile("vfrsub.vf       v24, v24, %[one]" :: [one]"f"(1.0f));      /* 1 - prev */
        asm volatile("vfmul.vv        v0,  v24, v8");                             /* apply sign */
#endif
...
/* ========================= LMUL = 4 =========================
 * v12 : x / |x| / r / P / final tanh
 * v8  : sign(x)
 * v16 : k / 2^k  (Cheby) | den (Schraudolph)
 * v24 : temp     (Cheby) | E-bit pattern (Schraudolph)
 */
static inline void vtanh_m4_strip(const float* inp, float* out, int N) {
    ...
    while (remaining > 0) {
        ...
        asm volatile("vfmv.v.f  v8,  %[one]" :: [one]"f"(1.0f));       /* required for sgn */
...
#  endif
        ...
        /* 1 - 2/(E+1) via vector–scalar ops */
        asm volatile("vfadd.vf        v24, v12, %[one]" :: [one]"f"(1.0f));     /* E+1 */
        asm volatile("vfrdiv.vf       v24, v24, %[two]" :: [two]"f"(2.0f));     /* 2/(E+1) */
        asm volatile("vfrsub.vf       v24, v24, %[one]" :: [one]"f"(1.0f));     /* 1 - prev */
        asm volatile("vfmul.vv        v12, v24, v8");
#endif
...
/* ========================= LMUL = 2 =========================
 * v4  : x / |x| / r / P / final tanh
 * v6  : sign(x)
 * v8  : k / 2^k (Cheby) | den (Schraudolph)
 * v10 : temp    (Cheby) | E-bit pattern (Schraudolph)
 */
static inline void vtanh_m2_strip(const float* inp, float* out, int N) {
    ...
    while (remaining > 0) {
        ...
        asm volatile("vfmv.v.f  v6,  %[one]" :: [one]"f"(1.0f));      /* required for sgn */
...
#  endif
        ...
        /* 1 - 2/(E+1) via vector–scalar ops */
        asm volatile("vfadd.vf        v10, v4,  %[one]" :: [one]"f"(1.0f));     /* E+1 */
        asm volatile("vfrdiv.vf       v10, v10, %[two]" :: [two]"f"(2.0f));     /* 2/(E+1) */
        asm volatile("vfrsub.vf       v10, v10, %[one]" :: [one]"f"(1.0f));     /* 1 - prev */
        asm volatile("vfmul.vv        v4,  v10, v6");
#endif
...
int main(void) {
    ...
    /* Optional check (kept disabled to match your current use) */
    // if (cid == 0) {
