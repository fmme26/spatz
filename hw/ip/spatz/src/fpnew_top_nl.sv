...
module fpnew_top import spatz_pkg::*; #(
  // FPU configuration
...
if(nl_concatenate) begin
    unique case(tag_i.nl_op_sel)
      EXPS: begin
        if ( opgrp_out_valid[fpnew_pkg::ADDMUL]) begin
      ...
      COSHS: begin
        if (opgrp_out_valid[fpnew_pkg::ADDMUL] && is_last_uop_q != 1'b1) begin
          operands_fconv[0]                   = opgrp_outputs[0].result;
          operands_fconv[1]                   = '0;
          operands_fconv[2]                   = '0;   
          nl_opmode_conv                      = 1'b0;
        ...
        if (opgrp_out_valid[fpnew_pkg::ADDMUL] && opgrp_outputs[0].tag.last_phase != 1'b0) begin
          is_last_uop_d                       = 1'b1;
        end
      
        if (opgrp_out_valid[fpnew_pkg::CONV] && opgrp_outputs[3].tag.nl_phase == NL_FPU_ISSUE_1) begin
        ...
        if (opgrp_out_valid[fpnew_pkg::CONV] && opgrp_outputs[3].tag.nl_phase== NL_FPU_ISSUE_0) begin
          nl_wr_en = 1'b1;
  ...
  for (genvar opgrp = 0; opgrp < int'(NUM_OPGROUPS); opgrp++) begin : gen_operation_groups
    ...
    always_comb begin : Arbiter_input_mux 
      nl_outputs[opgrp] = opgrp_outputs[opgrp];
        if (opgrp_outputs[opgrp].tag.nl_op_sel == TANHS && (opgrp == 0)) begin
          nl_outputs[opgrp].result = result_tanh;
        end else if ((opgrp_outputs[opgrp].tag.nl_op_sel == COS || opgrp_outputs[opgrp].tag.nl_op_sel == SIN)  && (opgrp == 0) ) begin
          nl_outputs[opgrp].result = result_tanh;
        end else begin
          nl_outputs[opgrp] = opgrp_outputs[opgrp];
        end
